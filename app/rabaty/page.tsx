...
import { BadgePercent, LockKeyhole, ArrowRight, Timer, Ticket, History, AlertCircle, CheckCircle2, Repeat, Calendar } from "lucide-react";
...
export default function RabatyPage() {
  ...
  useEffect(() => {
    ...
    const fetchKupony = async () => {
      const { data } = await supabase.from('kupony').select('*').eq('is_active', true);
  ...
  const handleExpire = (id: string, timestamp: number) => {
    ...
    setUsedCoupons(prev => {
      // Usuń stare użycie tego samego kuponu (jeśli wielorazowy), żeby zapisać najnowsze
  ...
  const availableCouponsList = dbCoupons.filter(c => {
    // Odfiltruj kupon jeśli ma restrykcje dnia, a dziś jest inny
    if (c.allowed_days && c.allowed_days.length > 0 && !c.allowed_days.includes(todayIndex)) {
      return false;
    }
    // Odfiltruj jeśli jest właśnie kliknięty
    if (activeCoupon?.id === c.id) return false;

    // Sprawdź w historii
    const useData = usedCoupons.find(uc => uc.id === c.id);
    if (useData) {
      if (!c.is_reusable) return false; // Jednorazowy i wykorzystany = znika na zawsze z dostępnych
      if (isUsedToday(useData.usedAt)) return false; // Wielorazowy, ale już użyty DZIŚ = znika do jutra
    }
    return true;
  });
