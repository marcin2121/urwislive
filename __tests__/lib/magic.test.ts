...
describe('askCrystalBall', () => {
  it('should return one of the predefined jokes', async () => {
    const response = await askCrystalBall('Test Product');
    const jokes = [
    ...
    assert.ok(jokes.includes(response), `Response should be one of the predefined jokes, got: ${response}`);
  });
});
